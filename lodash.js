...
// Bonus Points
let emptyObj = {};
const totalByMonth = _.reduce(purchasesByMonth,(sum,perMonth,month,x)=>{
  for (let i=0;i<perMonth.length;i++){
    console.log(sum)
    
    if (emptyObj.hasOwnProperty(perMonth[i].month)){
      emptyObj[perMonth[i].month] = 0
    } else {
      emptyObj = {...emptyObj, [perMonth[i].month]: 0}
    }
    return emptyObj
  }
},0); 
